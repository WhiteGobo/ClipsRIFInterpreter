...
(defrule RIFprocess_Execute
	...
	=>
	(println "Missing implementation of Execute")
	(set-error "Missing implementation of Execute")
...
(defmessage-handler RIFForall create-sentence ()
	(if (and (eq nil ?self:pattern) (eq RIFImplies (type ?self:formula)))
		then (return (send ?self:formula create-sentence)))
	(bind ?name (send ?self get-rulename))
	(bind ?lhs (send ?self get-lhs))
	(bind ?rhs (send ?self get-rhs))
	(return (printrule ?name "" ?lhs ?rhs)))
...
(defmessage-handler RIFFrame create-sentence ()
	(bind ?name (send ?self get-rulename))
	(bind ?ret (create$))
...
"))
	(foreach ?sntc (send ?self:sentences get-items)
		(bind ?sntcRet (send ?sntc create-sentence))
