...
(defmessage-handler RIFObj create-sentence (?salience)
	(if (eq nil ?salience) then (bind ?salience 0))
...
(defmessage-handler RIFImport create-rules (?salience)
	(return "")
...
(defmessage-handler RIFGroup create-rules (?prio)
	(if (neq nil ?self:priority) then (bind ?prio ?self:priority))
	(bind ?ret (create$ ";autogenerated group rules
"))
	(foreach ?sntc (send ?self:sentences get-items)
		(if (eq RIFGroup (type ?sntc))
			then
			(bind ?ret (append$ ?ret (send ?sntc create-rules ?prio)))
			else
			(bind ?sntcRet (send ?sntc create-sentence ?prio))
...
(defmessage-handler RIFDocument create-rules ()
	...
	(if (neq ?self:payload nil) then
		then (return (str-cat
			...
			(send ?self:payload create-rules nil)))
