...
static bool reg_true(char *lexical){
	int err;
	regex_t reg;
	err = regcomp(&reg, "^(1)|([tT]rue)|(TRUE)$",
			REG_EXTENDED | REG_NOSUB);
	if (err == 0){
		err = regexec(&reg, lexical, 0, NULL, 0);
	}
...
static bool reg_false(char *lexical){
	int err;
	regex_t reg;
	err = regcomp(&reg, "^(0)|([fF]alse)|(FALSE)$",
			REG_EXTENDED | REG_NOSUB);
	if (err == 0){
		err = regexec(&reg, lexical, 0, NULL, 0);
	}
...
bool castable_as_true(char* lexical, char *x, size_t xl){
	if (ISURI(x, xl, _XS_boolean_) || is_string_datatype(x, xl)){
		return reg_true(lexical);
	}
	return false;
}
bool castable_as_false(char* lexical, char *x, size_t xl){
	if (ISURI(x, xl, _XS_boolean_) || is_string_datatype(x, xl)){
		return reg_false(lexical);
	}
	return false;
}
